...
**フェーズ**: Phase 3完了 - 札幌市全域対応

**動いているもの**:
- `cd app && npm run dev` で起動
- PLATEAU 3D都市モデル（札幌市全10区対応）
- ヒグマ出没データ（種別別3Dモデルマーカー表示）
- ランディングページ（統計ダッシュボード）
- 日照シミュレーション

**札幌市10区**: 中央区・北区・東区・白石区・厚別区・豊平区・清田区・南区・西区・手稲区
...
- **データ**: 3D Tiles（PLATEAU配信サービス）、JSON（ヒグマデータ）
- **ホスティング**: Vercel（フロントエンド）、PLATEAU配信サービス（3D Tiles）
...
`app/.env.example` を参照してCesium ionトークンを設定（地形データ用）。
PLATEAUの3D Tilesは配信サービスから直接読み込むため、アセットIDは不要。
...
**更新履歴**:
- 2026-01-04: 札幌市全10区対応（PLATEAU配信サービスから直接読み込み）
